import pandas as pd
import re, codecs, os, subprocess, shutil, wxconv, glob
...
parellel_corpus_excel_path = 'data/ParallelCorpus.xlsx'
#3. name of the mrs dictionary data file
mrs_dictionary_filename = 'data/H_concept-to-mrs-rels.dat'
#4. file format of the usr files (txt/xlsx)
usr_ff = 'txt'
#5. combined USR file name
result_comb_usr = 'single-chapter2.txt'
...
#helper function to generate dataframe from parallel corpus
...
#     # d = {'sent_id':[],'hin_sent':[],'og_conc':[]}#,'num':[],'col4':[],'pos':[],'deprel':[],'col7':[],'col8':[],'col9':[],'type':[],'warning':[]}
#     d = {'sent_id': [], 'hin_sent': [], 'og_conc': [], 'warning': []}
...
def gen_combined_txtUSR(dir_path):
    d = {'sent_id':[],'hin_sent':[],'og_conc':[],'num':[],'col4':[],'pos':[],'deprel':[],'col7':[],'col8':[],'col9':[], 'col10':[], 'type':[],'warning':[]}
    res = []
    for path in os.listdir(dir_path):
        if os.path.isfile(os.path.join(dir_path, path)):
            fpath = dir_path.split('/')[-1] + '/' + path
            ...
            if len(lines)>=11:
                ...
                d['col10'].append(lines[9])
                d['type'].append(lines[10])
                if len(lines) == 12:
                    d['warning'].append(lines[11])
                else:
                    d['warning'].append('')
...
def gen_combined_USR(dir_path):
    ...
    for path in os.listdir(dir_path):
        if os.path.isfile(os.path.join(dir_path, path)):
                ...
                if len(df1.index)<10:
                    print(path)
                    for i in range(len(df1.index),11):
                        df1.loc[i] = ['']*len(list(df1))

                conc,num,col4,pos,deprel,col7,col8,col9,col10,type1,warning = [],[],[],[],[],[],[],[],[],[],[]
                for i in range(len(list(df1))):
                    conc.append(str(df1.iloc[1][i]))
                    num.append(str(df1.iloc[2][i]))
                    col4.append(str(df1.iloc[3][i]))
                    ...
                    col8.append(str(df1.iloc[7][i]))
                    col9.append(str(df1.iloc[8][i]))  
                    col10.append(str(df1.iloc[9][i]))           
                    type1.append(str(df1.iloc[10][i]))
                    if len(df1) == 12:
                        warning.append(str(df1.iloc[11][i]))
                    else:
                        warning.append('')
